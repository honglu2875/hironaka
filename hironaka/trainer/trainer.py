...
class Trainer(abc.ABC):
    ...
    def __init__(
        ...
        assert any(
            [role in self.config for role in ["host", "agent"]]
        ), "Must have at least one role out of ['host', 'agent'] in the config."
    ...
    def get_rollout(self, role: str, num_of_games: int, steps: int, er: Optional[float] = None) -> List:
        """
    ...
    def get_rho_for_pair(
        ...
        fused_game = FusedGame(
            host_candidate, agent_candidate, device=self.device, reward_func=self.reward_func, dtype=self.dtype
        )
        initial = sum(points.ended_batch_in_tensor)
        previous = initial
        total_steps = 0
        for i in range(max_steps):
            host_move, _ = fused_game.host_move(points, exploration_rate=0.0)
            fused_game.agent_move(
                points, host_move, scale_observation=self.scale_observation, inplace=True, exploration_rate=0.0
            )
    ...
    @torch.inference_mode()
    def count_actions(self, role: str, games: int, max_steps: int = 100, er: Optional[float] = None) -> torch.Tensor:
        """
    ...
    def copy(self):
        ...
        return self.__class__(  # pytype: disable=not-instantiable
            ...
            host_net=deepcopy(self.get_net("host")),
            agent_net=deepcopy(self.get_net("agent")),
    ...
    def _make_network(self, role: str, head_cls: Any, input_dim: int, output_dim: int, pretrained_net: nn.Module):
        ...
        if pretrained_net is not None:
            ...
            if len(self.use_cuda_ids) > 1 and not isinstance(pretrained_net, DummyModule):
                net = DataParallel(self._create_network(head, net_arch, input_dim, output_dim), self.use_cuda_ids)
            else:
    ...
    def _make_replay_buffer(self, role: str, output_dim: int):
        ...
        if self.use_replay_buffer:  # Ignore if `deactivate` is True
            ...
            if role == "host":
                ...
                input_shape = {"points": self.feature_shape, "coords": (self.dimension,)}
            else:
                raise Exception("Impossible code path.")

            replay_buffer = self.replay_buffer_dict[cfg["type"]](
                input_shape=input_shape, output_dim=output_dim, device=device, dtype=self.dtype, **cfg
            )
    ...
    def _generate_random_points(self, samples: int, device: torch.device) -> TensorPoints:
        pts = torch.randint(
            self.max_value + 1, (samples, self.max_num_points, self.dimension), dtype=self.dtype, device=device
        )
