...
class HironakaNet(nn.Module):
    ...
    def __init__(self, dim=3):
        ...
        self.fc3 = nn.Linear(32,
                             2 ** dim - dim)  # output: there are 8 = 2^3 action of choosing subsets, and the last number is the expected steps
...
class MCTS:
    def __init__(self, state, env, nn, max_depth=15, c_puct=0.5):
        ...
        self.initial_state = state if isinstance(state, TensorPoints) else TensorPoints(state.points,
                                                                                        max_num_points=10)  # Turn points into PointsTensor
    ...
    def _search(self, s: TensorPoints, depth=0):
        ...
        if all(v == 0 for v in self.Q[hashed_s]):
            ...
            for action in range(0, self.nn.choices):
                u = self.Q[hashed_s][action] + self.c_puct * self.P[hashed_s][action] * math.sqrt(
                    sum(self.N[hashed_s])) / (
        ...
        self.Q[hashed_s][this_action] = (self.N[hashed_s][this_action] * self.Q[hashed_s][
            this_action] + current_reward) / (
